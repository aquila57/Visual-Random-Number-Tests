/*
eeglgen.c Version 0.1.0. Random dots, eegl generator
...
int main()
   ...
   while(i--)   /* generating loop */
      ...
      x = eegl(ee) >> 27;   /* set random column 0 to 31 */
      y = eegl(ee) >> 27;   /* set random row    0 to 31 */
      z = eegl(ee) >> 27;   /* set random row    0 to 31 */
      /* if one, erase the pixel at x,y */
      bit = eegl(ee) >> 31;      /* choose black or white dot */
      cube[x][y][z] = bit;
      } /* for each sample */
