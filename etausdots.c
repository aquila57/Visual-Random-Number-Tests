/*
etausdots.c Version 0.1.0. Paint random dots on an X Windows screen
...
int main()
   ...
   while(xx->runflg)   /* processing loop */
      ...
      /* this illustrates how to use etauspwr() */
      x = etauspwr(et,9);   /* set random column 0 to 511 */
      y = etauspwr(et,9);   /* set random row    0 to 511 */
      /* if one, erase the pixel at x,y */
      if (etausbit(et)) XSetForeground(xx->dpy, xx->gc, xx->whiteColor);
