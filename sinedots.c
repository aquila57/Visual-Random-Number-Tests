...
int main()
   ...
   while(xx->runflg)   /* processing loop */
      ...
      sinx = sin(radian) * 512.0;
      x = (int) floor(sinx);   /* set random column 0 to 511 */
      radian = eeglunif(ee) * M_PI;
      sinx = sin(radian) * 512.0;
      y = (int) floor(sinx);   /* set random row    0 to 511 */
      radian = eeglunif(ee) * M_PI;
      sinx = sin(radian);   /* 0.0 <= sinx <= 1.0 */
      /* if 0.5 <= sinx <= 1.0, erase the pixel at x,y */
      if (sinx >= 0.5) XSetForeground(xx->dpy, xx->gc, xx->whiteColor);
