/*
rudots.c Version 0.1.0. Random dots, RANDU generator
...
int main()
   ...
   while(xx->runflg)   /* processing loop */
      ...
      xx->seed *= EMM;
      x = xx->seed >> 23;   /* set random column 0 to 511 */
      xx->seed *= EMM;
      y = xx->seed >> 23;   /* set random row    0 to 511 */
      /* if one, erase the pixel at x,y */
      xx->seed *= EMM;
      bit = xx->seed >> 31;      /* choose black or white dot */
      /* If bit set, paint the pixel at x,y white */
      if (bit)        /* if white color */
